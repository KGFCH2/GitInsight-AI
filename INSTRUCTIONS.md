...
## 🛠️ Root Directory
- 🏗️ **`ARCHITECTURE.md`**: Detailed system design, data flow, and image optimization logic.
...
- ⌨️ **`tsconfig.json` / `tsconfig.node.json`**: TypeScript compiler settings for app and build tools.
...
- 🖌️ **`index.css`**: Global styles, design tokens, and glassmorphism utilities.
- 🏷️ **`vite-env.d.ts`**: TypeScript definitions for Vite environment variables.

### 📁 `src/lib/` (The Engine)
- ⚙️ **`api.ts`**: **Core Logic Hub**. Handles API calls, scoring, and registry logic.
- 📄 **`pdf.ts`**: **Reporting Engine**. Generates professional PDF Audit Reports.
- 🏷️ **`types.ts`**: Global TypeScript interfaces and domain models.
- 🛠️ **`utils.ts`**: Shared utility functions and Tailwind class merging logic.
...
- 🛡️ **`AdminDashboard.tsx`**: **Commander Terminal**. Registry and history management.
...
- ⚖️ **`Terms.tsx`**: Legal terms and conditions of service.
...
- 🗂️ **`Card.tsx`**: The base glassmorphism card component used everywhere.
- 🦶 **`Footer.tsx`**: Global site footer with credits and legal links.
- 🖼️ **`Layout.tsx`**: The main page wrapper and navigation coordinator.
...
- 🖱️ **`button.tsx` / `button-variants.ts`**: Flexible, theme-aware button system.
- ⌨️ **`input.tsx`**: Styled text inputs with glassmorphism support.
...
- 🏗️ **`setup.ts`**: Configuration for the Vitest testing environment.
