...
function SectionHeading({ icon: Icon, title }: { icon: React.ComponentType<{ className?: string }>; title: string }) {
  return (
    <div className="hover-pop group flex items-center gap-3">
      ...
      <h2 className="text-gradient font-display text-2xl font-bold">{title}</h2>
    </div>
  );
}
...
function Step({ n, title, children }: { n: number; title: string; children: React.ReactNode }) {
  return (
    <li className="hover-pop group flex gap-4 rounded-2xl border border-border bg-card-grad p-5">
      ...
      <div>
        <div className="text-gradient font-display text-lg font-bold">{title}</div>
